...
- `server/lib/penny-agent/pipeline.ts` owns the end-to-end receipt-analysis flow.
- `server/lib/openai-receipt.ts` owns receipt extraction.
- `server/lib/penny-agent/run.ts` owns the Penny tool loop.
- `server/lib/penny-agent/stream.ts` owns streamed Penny jobs and chat re-attach.
...
6. Receipt analysis goes through `server/lib/penny-agent/pipeline.ts`, which streams progress, saves chat state, and persists successful runs.
